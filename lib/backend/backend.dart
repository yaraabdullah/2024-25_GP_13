...
import 'schema/available_slots_record.dart';
...
export 'schema/available_slots_record.dart';
...
/// Functions to query AvailableSlotsRecords (as a Stream and as a Future).
Future<int> queryAvailableSlotsRecordCount({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      AvailableSlotsRecord.collection(parent),
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<AvailableSlotsRecord>> queryAvailableSlotsRecord({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
    ...
    queryCollection(
      AvailableSlotsRecord.collection(parent),
      AvailableSlotsRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<AvailableSlotsRecord>> queryAvailableSlotsRecordOnce({
  DocumentReference? parent,
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
...
Future maybeCreateUser(User user) async {
  final userRecord = StudentsRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await StudentsRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createStudentsRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      StudentsRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createStudentsRecordData(email: email));
}
